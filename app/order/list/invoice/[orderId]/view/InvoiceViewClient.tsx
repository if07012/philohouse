...
export default function InvoiceViewClient({ initialData }: InvoiceViewClientProps) {
  ...
  useEffect(() => {
    if (initialData || isCheckingAuth || !orderId) return;
    ...
    fetchData();
  }, [initialData, isCheckingAuth, orderId]);
  ...
  const handleMarkAsSent = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/orders/${encodeURIComponent(order["Order ID"])}/invoice-status`,
        {
          method: "PATCH",
