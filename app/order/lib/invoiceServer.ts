...
function parseInvoiceRows(
...
): Pick<InvoiceData, "extraItems" | "discount" | "subtotal" | "discountAmount" | "total"> & {
  cookieDetails: InvoiceData["order"]["cookieDetails"];
} {
  ...
  for (const row of data) {
    ...
    if (c1 === "Item" && c2 === "Size" && c3 === "Qty") inItems = true;
    else if (inItems && c1 && c1 !== "Subtotal" && c1 !== "Diskon" && c1 !== "Total") {
      const qty = Number(c3) || 0;
      const subtotalVal = Number(c4) || 0;
      if (c2 === "-" || c2 === "") {
        extraItems.push({ name: c1, quantity: qty, unitPrice: qty > 0 ? subtotalVal / qty : 0 });
      } else {
        cookieDetails.push({
          "Cookie Name": c1,
          Size: String(c2),
  ...
  return {
    cookieDetails,
    extraItems,
    discount: discountAmount > 0 ? { type: discountType, value: discountValue } : undefined,
    subtotal,
    discountAmount,
    total,
  };
...
/** Get invoice data from an already-loaded spreadsheet doc (for batch loading) */
export async function getInvoiceDataFromDoc(
  doc: Awaited<ReturnType<typeof getGoogleSheet>>,
  orderId: string
): Promise<
  Pick<InvoiceData, "extraItems" | "discount" | "subtotal" | "discountAmount" | "total"> & {
    cookieDetails: InvoiceData["order"]["cookieDetails"];
...
export async function getInvoiceByOrderId(
  ...
  try {
    ...
    for (const row of data) {
      const c1 = String(row.Col1 || "").trim();
      const c2 = String(row.Col2 ?? "");
      if (c1 === "Date") orderDate = String(c2);
    ...
    const parsed = parseInvoiceRows(data);
    ...
    return {
      order: {
        ...
        Total: parsed.total,
        cookieDetails: parsed.cookieDetails,
      },
